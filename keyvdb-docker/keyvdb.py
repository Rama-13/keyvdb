...
from prometheus_client import multiprocess, Counter, Gauge, Histogram
import metrics
...
app = Flask(__name__)
metrics.setup(app)
metrics_1 = PrometheusMetrics(app)
metrics_1.info('app_info', 'Application info', version='1.0.3')
lock = threading.Lock()
...
metrics_1.register_default(
    metrics_1.counter(
    'by_endpoint_counter', 'Request count by endpoints',
    labels={'endpoint': lambda: request.endpoint}
),
metrics_1.summary('requests_by_status', 'Request latencies by status',
                 labels={'status': lambda r: r.status_code}),
metrics_1.histogram('requests_by_status_and_path', 'Request latencies by status and path',
